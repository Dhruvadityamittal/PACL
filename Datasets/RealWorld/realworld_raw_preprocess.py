...
def cal_xyz(zip_file, csv, min_time_len):
    ...
    f = zip_file.open(csv)
    df = pd.read_csv(f)
    # t = df['attr_time']  # Adding time (By Dhruv)
    # t = np.array(t)
    ...
    xyz = np.concatenate((x, y, z), axis=1) # Adding time (By Dhruv)
    xyz = xyz[:min_time_len, :]  # Taking the length of the sensor having minimum length
    f.close()
    return xyz
...
def cal_xyz_acc(zip_file, i, sub_id, label):
    ...
    min_time_len = cal_min_time_len(readme_file)  # Calculates the minumum number of sensor values for a particular activity.
    ...
    for b_part in body_parts_list:
        if i == 1:
            ...
            if con1 or con2 or con3 or con4 or con5:
                csv_file_name = "acc_{}_2_{}.csv".format(label, b_part) # csv files are named differently here.
        ...
        if csv_file_name in zip_file.namelist():
            ...
            if b_part == "chest":  # chest is the first body part just to initialize the array.
                xyz_acc = xyz
            else:
            ...
            xyz = np.full([min_time_len, 3], np.nan)  # If data is not present then add none array.
            if b_part == "chest":   # Why?
                xyz_acc = xyz
            else:
                xyz_acc = np.concatenate((xyz_acc, xyz), axis=1)
