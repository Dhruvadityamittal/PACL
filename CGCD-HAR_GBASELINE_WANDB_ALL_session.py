...
pth_rst_exp_log_step_1 = pth_rst_exp_step_1+f'results_{args.dataset}_{args.model}'
...
pth_rst_exp_log_step_2 = pth_rst_exp_step_2  + f"results_{args.dataset}_{args.model}"

if(args.session_split):
    pth_rst_exp_log_step_1 += "_session.log"
    pth_rst_exp_log_step_2 += "_session.log"
else:
    pth_rst_exp_log_step_1 += ".log"
    pth_rst_exp_log_step_2 += ".log"

if(args.sampling == 'kde'):
    
    pth_rst_exp_log_step_1 = pth_rst_exp_log_step_1[:-4] + '_kde' +'.log'
    pth_rst_exp_log_step_2 = pth_rst_exp_log_step_2[:-4] + '_kde' +'.log'
...
if __name__ == '__main__':
    ...
    for lr, kd_weight, pa_weight,contrastive_weight, fold in itertools.product(lrs, kd_weights, pa_wights,contrastive_weights, folds):
        ...
        dset_tr_0 = dataset.load(name=args.dataset, root=pth_dataset, mode='train_0', windowlen=window_len, 
                                autoencoderType=None, standardize=args.standarization_prerun, fold=fold, session_split = args.session_split)
        ...
        flops, macs, params = calculate_flops(model=model, 
                                      input_shape=(256, 3, 100),
                                      output_as_string=True,
                                      output_precision=4)
        print("FLOPs:%s   MACs:%s   Params:%s \n" %(flops, macs, params))
        
        inputs =  torch.randn(256, 3, 100).to(device)
        with profile(activities=activities, record_shapes=True, with_flops=True, with_modules=True) as prof:
        ...
        batches_per_secs, mem_per_secs, peak_memories = [], [], [] 
        ...
        for epoch in range(0, args.nb_epochs):
            ...
            with profile(activities=activities, schedule=torch.profiler.schedule(wait=1, warmup=1, active=2), on_trace_ready=trace_handler, with_flops=True) as p:
                ...
                for batch_idx, (x, y, z) in enumerate(dlod_tr_0):
                    ...
                    # Only Offline, Online Finetuning, G-Baseline Models donot have contrastive losses.
                    if(args.contrastive_loss_type not in ['Offline', 'Online_Finetuning', 'G-Baseline', 'EWC']):
                ...
                batches_per_sec = (batch_idx +1) / duration
                peak_memory = torch.cuda.max_memory_allocated(device) / (1024 ** 2)  # in MB

                memory_per_sec = peak_memory / duration
            ...
            with torch.no_grad():
                ...
                print('Valid Acc: {:.4f}'.format(acc_0))
                if args.use_wandb:
            ...
            if best_acc < acc_0:
                ...
                torch.save({'model_pa_state_dict': model.state_dict(), 'proxies_param': criterion_pa.proxies},
                        '{}{}_{}_best_windowlen_{}_embedding_size_{}_alpha{}_mrg_{}_version_{}_step_{}_fold_{}_session_split_{}.pth'.format(
                            pth_rst_exp_step_1, args.dataset, args.model, window_len, args.sz_embedding, args.alpha, args.mrg, version, step, fold, args.session_split))
                es_count = 0
            else:
            ...
            # Batches/Memory per second  
            print(f"Batches/sec: {batches_per_sec:.2f} | Peak Mem: {peak_memory:.2f} MB | Mem/sec: {memory_per_sec:.2f} MB/s")
            batches_per_secs.append(batches_per_sec)
            mem_per_secs.append(memory_per_sec)
        ...
        try:
            print(f"Mean --> Batches/sec: {np.mean(batches_per_sec):.2f} | Peak Mem Max: {np.max(peak_memory):.2f} MB | Mem/sec: {np.mean(memory_per_sec):.2f} MB/s")
        except:
            print("Information related to batches and memory per sec for step 1 not calculated. Train the model to get the values ....")
        ...
        pth_pth = '{}{}_{}_best_windowlen_{}_embedding_size_{}_alpha{}_mrg_{}_version_{}_step_{}_fold_{}_session_split_{}.pth'.format(
            pth_rst_exp_step_1, args.dataset, args.model, window_len, args.sz_embedding, args.alpha, args.mrg, version, step, fold, args.session_split)
        ...
        # EWC
        if(args.contrastive_loss_type == 'EWC'):
            model.train()
            with torch.no_grad():
                utils_CGCD.estimate_fisher(model, dset_ev, len(dset_ev.xs),criterion_pa, ewc_gamma=1.)
        ...
        if(args.visualize_proxies):
            utils_CGCD.visualize_proxy_anchors(model, dlod_tr_0, criterion_pa.proxies, args.dataset, args.sz_embedding, nb_classes, step, dim_reduction_method)
        

        ###
        print("\n--------------------- Initial Step Test --------------------- \n")
        nb_classes_initial_test = dset_test_0.nb_classes()
        nb_subjects_initial_test = dset_test_0.nb_subjects()
        print("\n")
        ...
        if args.only_test_step2 != True:
            ...
            if load_exempler:
                ...
                expler_s = torch.load(pth_rst_exp_step_1 + 'expler_s_tensor_model_{}_{}_windowlen_{}_sz_embedding_{}_alpha{}_mrg_{}_PA_{}_KD_{}_lr_{}_wd_{}_fold_{}_session_split_{}.pt'.format(args.dataset, args.model, window_len, args.sz_embedding, args.alpha, args.mrg, args.pa_weight, args.kd_weight, args.lr, args.weight_decay, fold, args.session_split), map_location=torch.device(device))
            else:
                ...
                with torch.no_grad():
                    ...
                    torch.save(expler_s, pth_rst_exp_step_1 + 'expler_s_tensor_model_{}_{}_windowlen_{}_sz_embedding_{}_alpha{}_mrg_{}_PA_{}_KD_{}_lr_{}_wd_{}_fold_{}_session_split_{}.pt'.format(args.dataset, args.model, window_len, args.sz_embedding, args.alpha, args.mrg, args.pa_weight, args.kd_weight, args.lr, args.weight_decay, fold, args.session_split))
        ...
        for epoch in range(0, args.nb_epochs):
            ...
            if args.contrastive_loss_type not in  ["Online_Finetuning", 'Offline', 'Online_Finetuning_NCE', 'Offline_NCE', 'G-Baseline_NCE_WFR', 'EWC']:
                ...
                # x_sampled = x_sampled
                # y_sampled = y_sampled
                dset_tr_now.xs.extend(x_sampled)
                dset_tr_now.ys.extend(y_sampled)
                dlod_tr_now = torch.utils.data.DataLoader(dset_tr_now, batch_size=args.sz_batch, shuffle=True, num_workers=args.nb_workers, pin_memory=True, worker_init_fn=lambda x: np.random.seed(42), drop_last = True)
            ...
            for batch_idx, (x, y, z) in pbar:
                ...
                if args.contrastive_loss_type not in  ["Online_Finetuning", 'Offline', 'Online_Finetuning_NCE', 'Offline_NCE', 'EWC']:
            ...
            if total_data > 0:
                print(f"\n------------------- Epoch {epoch} ---------------------\n") 
                print("Train Total Loss {:.4f}, PA Loss {:.4f}, KD Loss {:.4f} Contrastive Loss {:.4f}".format(
            ...
            if (acc_a > best_weighted_acc):
                ...
                torch.save({'model_pa_state_dict': model_now.state_dict(), 'proxies_param': criterion_pa_now.proxies},
                            '{}{}_{}_model_best_windowlen_{}_sz_embedding_{}_alpha_{}_mrg_{}_lr_{}_wd_{}_step_{}_fold_{}_session_split_{}_sampling_{}.pth'.format(
                                pth_rst_exp_step_2, args.dataset, args.model, window_len, args.sz_embedding,
                                args.alpha, args.mrg,  args.lr, args.weight_decay,
                                str(step), fold, args.session_split, args.sampling))
            else:
                es_count += 1
        ...
        try:
            ...
            print("Information related to batches and memory per sec for step 2 not calculated. Train the model to get the values ....")

        # Prepare for testing the final model
        pth_pth_test = '{}{}_{}_model_best_windowlen_{}_sz_embedding_{}_alpha_{}_mrg_{}_lr_{}_wd_{}_step_{}_fold_{}_session_split_{}_sampling_{}.pth'.format(
            pth_rst_exp_step_2, args.dataset, args.model, window_len, args.sz_embedding,
            args.alpha, args.mrg, args.lr, args.weight_decay, str(step), fold, args.session_split, args.sampling)
        ...
        if args.model == 'resnet18':
            ...
            repo = 'OxWearables/ssl-wearables'
            model_test = torch.hub.load(repo, 'harnet5', class_num=nb_classes_now, pretrained=True).to(device)
            del model.classifier
        ...
        # Visualizing Incremental Step Data and Proxies after Incremental Step

        if(args.visualize_proxies):
            utils_CGCD.visualize_proxy_anchors(model_test, dlod_tr_now, criterion_pa_test.proxies, args.dataset, args.sz_embedding, nb_classes_now, step, dim_reduction_method)
        

        print("\n--------------------- Testing Step --------------------- \n")

        # Evaluate Step 2 test dataset on Stage 1 trained model
        print('==> Evaluation of Step 1 Dataset on Step 2 Model')
        model.eval()  # Set model to evaluation mode
        with torch.no_grad():
            # Extract features from the test data
            feats, _ = utils_CGCD.evaluate_cos_(model_test, dlod_test_0)
            ...
            y_temp = torch.tensor(dlod_test_0.dataset.ys)
            ...
            stage_2_acc_0 = accuracy_score(np.array(dlod_test_0.dataset.ys)[old_classes].reshape(-1), preds[old_classes].reshape(-1))
            stage_2_acc_1 = accuracy_score(np.array(dlod_test_0.dataset.ys)[new_classes].reshape(-1), preds[new_classes].reshape(-1))
            stage_2_f1_0 = f1_score(np.array(dlod_test_0.dataset.ys)[old_classes].reshape(-1), preds[old_classes].reshape(-1), average='macro')


        # Print the Step 2 accuracy results
        print('==> Evaluation of Step 2 Dataset on Step 2 Model')
