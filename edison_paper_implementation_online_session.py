...
def print_gpu_memory_usage():
    ...
    for obj in gc.get_objects():
        if torch.is_tensor(obj) and obj.is_cuda:
            ...
            total_memory += memory
            print(f"Tensor size: {obj.size()}, Memory: {memory / 1024**2:.2f} MB")
    print(f"Total GPU memory used by tensors: {total_memory / 1024**2:.2f} MB\n")
...
parser.add_argument('--lr', default=1e-6, type=float, help='Learning rate setting')  #1e-4
...
parser.add_argument('--bn-freeze', default=False, type=bool, help='Batch normalization parameter freeze')
...
pth_rst_exp_step_1 = f'{os.getcwd()}/Saved_Models/Initial/Edison/' + args.dataset + '/' # + args.model + '_sp_' + str(args.use_split_modlue) + '_gm_' + str(args.use_GM_clustering) + '_' + args.exp
...
pth_rst_exp_step_2 = f'{os.getcwd()}/Saved_Models/Incremental/Edison/' + args.dataset + '/'
    ...
    for fold in folds:
        ...
        if(args.use_wandb):
            ...
            wandb.init(
                ...
                project="LAPNET-HAR-Our-Architecture",
        ...
        if(args.model=='tinyhar'):
            param_groups = [
                ...
                {'params': model.model.prediction.parameters() if gpu_id != -1 else model.model.prediction.parameters(), 'lr': float(args.lr) * 0.1},]
        else:
            param_groups = [
        ...
        for epoch in range(0,args.nb_epochs):  #args.nb_epochs
            ...
            if args.warm > 0:
                if gpu_id != -1:
                    if(args.model=='tinyhar'):
                        unfreeze_model_param = list(model.model.prediction.parameters()) 
            ...
            for batch_idx, (x, y, z) in enumerate(dlod_tr_0):
                ...
                for clas in sorted(np.unique(y_support)):
                    p_mean = torch.mean(x_support_embeddings[y_support==clas],dim=0)
                    prototypes[clas] = p_mean
                    counters[clas] = len(x_support_embeddings[y_support==clas])
                z_proto = torch.stack(list(prototypes.values()))
                ...
                loss = ce_loss + 10*contrastive_losses
            ...
            print(f"\n------------------- Epoch {epoch} ---------------------")     
            print("Total  {} Contrastive  {} CE  {} Acc {:.4f}".format(epoch,total_loss_per_epoch/nsteps, total_contrastive_losses/nsteps, total_ce_loss/nsteps, total_accuracy/nsteps))
            ...
            with torch.no_grad():
                for x, y, z in dlod_ev:
                    ...
                    for clas in sorted(np.unique(y_train_tensor.cpu().numpy())):
                        p_mean = torch.mean(x_train_tensor_embeddings[y_train_tensor.cpu().numpy()==clas],dim=0)
                    ...
                    z_proto_val = torch.stack(list(val_prototypes.values()))
            ...
            if(best_acc<val_accuracy):
                ...
                tuple_list = [(key, value.to('cpu').detach().numpy()) for key, value in best_prototypes.items()]
            ...
            if(es_count==10 or int(val_accuracy) == 1):
        ...
        # dlod_tr_only_new = torch.utils.data.DataLoader(dset_tr_only_new, batch_size=20, shuffle=True, num_workers=args.nb_workers, worker_init_fn=lambda x: np.random.seed(42))

        dlod_tr_only_new = dset_tr_now
        ...
        for epoch in range(0,args.nb_epochs):  #args.nb_epochs
            ...
            if args.warm > 0:
                if gpu_id != -1:
                    if(args.model=='tinyhar'):
                        unfreeze_model_param = list(model_now.model.prediction.parameters()) 
            ...
            for batch_idx, (x, y, z) in enumerate(dlod_tr_now):
            ...
            print(f"\n------------------- Epoch {epoch} ---------------------\n") 
            print("CE Loss {:.5f} Contrastive Loss {:.5f} Accuracy ".format(epoch, total_ce_loss/nsteps, total_contrastive_loss/nsteps ))
        ...
        if(args.model == 'resnet18'):
            ...
            model_test = ModelGen_new(cfg).create_model().to(device)
        if(args.model == 'tinyhar'):
        ...
        stage_2_acc_0, stage_2_f1_0, stage_2_acc_1, _, _, _ = get_accuracy_old_new(model_test, dlod_test_0, nb_classes, nb_classes_now, trained_prototypes_dict)
        ...
        if(args.use_wandb):
            ...
            wandb.finish()
    results_df.to_csv('results_edison_Resnet_{}1.csv'.format('_'.join(args.dataset), index=False))
