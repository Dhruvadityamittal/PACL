...
def contrastive_loss(embeddings, labels, prototypes, balance):
    ...
    try:
        prototypes = {key: torch.from_numpy(tensor).to('cuda') for key, tensor in prototypes.items()}
    except:
        prototypes = {key: tensor.to('cuda') for key, tensor in prototypes.items()}

    prototypes_values = torch.stack(list(prototypes.values())).to(device)
    
    prototypes_keys = torch.from_numpy(np.array(list(prototypes.keys()))).float().to(device)

    # print(prototypes_keys)
    
    prototypes_keys = torch.tensor(list(prototypes.keys())).to(device)

    
    embeddings = torch.cat((embeddings.to(device),prototypes_values),0)
    labels = torch.cat((labels.to(device),prototypes_keys),0)
    ...
    return loss.mean().to('cuda:0')
# def contrastive_loss(embeddings, labels, prototypes, balance):
#     device = 'cuda' if torch.cuda.is_available() else 'cpu'
#     labels = torch.argmax(labels, dim=1)
#     prototypes_values = torch.from_numpy(np.array(list(prototypes.values()))).float().to(device)
#     prototypes_keys = torch.from_numpy(np.array(list(prototypes.keys()))).long().to(device)
    
#     embeddings = torch.cat((embeddings, prototypes_values), 0).to(device)
#     labels = torch.cat((labels, prototypes_keys), 0).to(device)

#     labels_numpy = labels #.cpu().numpy()
#     all_pairs = np.array(list(combinations(range(len(labels_numpy)), 2)))
#     all_pairs = torch.LongTensor(all_pairs).to(device)

#     positive_mask = (labels_numpy[all_pairs[:, 0]] == labels_numpy[all_pairs[:, 1]])
#     negative_mask = (labels_numpy[all_pairs[:, 0]] != labels_numpy[all_pairs[:, 1]])

#     positive_pairs = all_pairs[positive_mask]
#     negative_pairs = all_pairs[negative_mask]

#     if balance and len(positive_pairs) < len(negative_pairs):
#         negative_pairs = negative_pairs[torch.randperm(len(negative_pairs))[:len(positive_pairs)]]
...
def get_accuracy_old_new(model, dload_test, nb_classes, nb_classes_now, trained_prototypes_dict):
    ...
    with torch.no_grad():  
        for x, y, z in dload_test:
            ...
            old_class_targets = [cl for cl in range(nb_classes)]
            new_class_targets = [cl for cl in range(nb_classes, nb_classes_now)]
            
            f1_o = f1_score(predicted_ys[actual_ys<nb_classes], actual_ys[actual_ys<nb_classes], labels = old_class_targets , average='macro')
            # f1_o = np.mean(f1_score(predicted_ys[actual_ys<nb_classes], actual_ys[actual_ys<nb_classes], average=None)[:nb_classes])
            ...
            # f1_n = np.mean(f1_score(predicted_ys[actual_ys>=nb_classes], actual_ys[actual_ys>=nb_classes], average=None)[nb_classes:])
            f1_n = f1_score(predicted_ys[actual_ys>=nb_classes], actual_ys[actual_ys>=nb_classes], labels = new_class_targets , average='macro')
    ...
    with torch.no_grad():  
        for x, y, z in dload_test:
            ...
            for c in trained_prototypes_dict.keys():
                ...
                # z_proto_test = torch.from_numpy(trained_prototypes_dict[c][None,:]).float().to(device)# Adding None just increases the shape from (128,) -> (1,128)
                z_proto_test = trained_prototypes_dict[c][None,:].to(device)
                dist_test = compute_euclidean(embeddings_test,z_proto_test)
                #print(np.shape(dist))
                dists_test[:,c] = torch.squeeze(dist_test)
